...
> 在线服务状态：`smedc-mcp-launcher@0.6.0`、浏览器登录和公开 HTTPS MCP 边界是当前打包契约。
...
可见并标为仅管理员可用；已知为 employee 时，agent 不得读取或转换本地文件。launcher 0.6.0
...
唯一批准的 launcher 包是 `smedc-mcp-launcher@0.6.0`。禁止使用 npm `latest`、未固定版本或
launcher 自更新。
